...
/* UserDetailService에서 사용자의 이름을 기분으로 조회한 결과가
* 반환되는 사용자 타입으로 해당 개체에 조회된 사용자의 정보가 담겨서 세션에 저장할 수 있다.
* */
...
public class AuthDetails implements UserDetails {
    ...
    * UsernamePasswordAuthenticationToken에 사용자의 권한 정보를 넣을 때 사용한다.*/
    @Override // 제네릭한 익스텐드한 콜렉션으로 가지고오겠다.
    public Collection<? extends GrantedAuthority> getAuthorities() {

        Collection<GrantedAuthority> authorities = new ArrayList<>();

        loginUserDTO.getRole().forEach(role -> authorities.add(() -> role)); //롤을 반환 해주겠다
    ...
    /* 잠겨있는 계정을확인하는 메소드로 false이면 해당 계정을 사용할 수 없다.
    * 비밀번호 반복 실패로 일시적인 계정 lock인 경우 혹은 오랜 시간 비 접속으로 휴먼 처리*/
    ...
    /* 탈퇴 게정 여부를 표현하는 메소드 false 면 해당 계정을 사용할 수 없다.
    * 보통 데이터 삭제는 즉시 하는 것이 아닌 열정 기간 보관 후 삭제한다. */
    @Override
